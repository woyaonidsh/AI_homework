...
from trainer_es import Trainer
from fitness import get_score
from EC import generatic
                        ...
                        if click_point is not None:  # 判断落子是否有效
                            if checkerboard.can_drop(click_point):  # 判断是否可落子
                                ...
                                if winner is None:
                                    ...
                                    action, p_action = GA.select_action(AI_point, checkerboard.checkerboard)  # AI落子位置
                                    ...
                                    if winner is not None:
                                        ...
                                        # 计算当前reward
                                        all_reward = GA.compute_reward(game_reward, game_winner)
                                        ...
                                        if total_count % 10 == 0 and total_count != 0:
                                            # 构建数据集
                                            dataset = GA.data_loader(stack, batch_size=argus.batch_size)
                                            # 开始训练模型
