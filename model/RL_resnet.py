...
class RL_model(nn.Module):
    def __init__(self, line_board, hidden_dim, out_layer, resnet18):
        ...
        self.conv1 = nn.Conv2d(in_channels=256, out_channels=out_layer, kernel_size=self.out_dim, stride=1)
        ...
        # 计算棋盘得分的线性层
        self.value_layer1 = nn.Linear(out_layer, hidden_dim)
        self.value_layer2 = nn.Linear(hidden_dim, hidden_dim)
        self.value_layer3 = nn.Linear(hidden_dim, 1)

    def forward(self, x):
        x = self.resnet(x)

        # 计算当前棋局得分
        value = self.conv1(x).view(-1, self.out_layer)
        value = self.leakyrelu(self.value_layer1(value))
        value = self.leakyrelu(self.value_layer2(value))
        value = self.tanh(self.value_layer3(value))
